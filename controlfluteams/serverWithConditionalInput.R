...
shinyServer(function (input, output#, 
  ...
  output$selectUI <- renderUI({ 
    selectInput("team", "Select your school", as.character(mydata()$school))
  })
  
  #######
  output$plot1 <- renderPlot({
    TeamFun(team = as.numeric(input$teamExplorer), data = mydata(), bar=FALSE)
    })
  ...
  output$plot2 <- renderPlot({
    ...
    TeamFun(team = as.numeric(input$team2), data = mydata(), bar=TRUE)
  })
  
  #######
  output$plot3 <- renderPlot({
    TeamFun(team = as.numeric(input$teamConsentFormReturn), data = mydata(), cf=TRUE, bar=FALSE)
  })
  ...
  output$plot4 <- renderPlot({
    ...
    TeamFun(team = as.numeric(input$teamConsentFormReturn), data = mydata(), cf=TRUE, bar=TRUE)
  })
  
  #######
  output$plot5 <- renderPlot({
    
    x <- ir[which(as.numeric(ir$team2014) == as.numeric(input$teamByGrade) ),]
  ...
  output$text1 <- renderText({
    paste("Team", input$teamExplorer, "immunization rate overview")
  })
  
  ########
  output$text2 <- renderText({
    paste("Team", input$teamConsentFormReturn, "consent form return rate overview")
  })
