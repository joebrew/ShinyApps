...
load("july7.RData") #maps
load("cen.df.RData") #census data
load("ts.RData") # time series made from data
load("preds.RData") #Subho's model data

preds$response.name <- ifelse(preds$resp == 1,
                              "interior hazard",
                              ifelse(preds$resp == 2,
                                     "exterior hazard",
                                     ifelse(preds$resp == 3,
                                  ...
                                  ifelse(preds$model == 3,
                                         ...
                                         ifelse(preds$model == 4,
                                                ...
                                                ifelse(preds$model == 5,
                                                       "naive Bayes",
                                                       ifelse(preds$model == 6,
                                                              "random forest",
                                                              NA))))))

preds$fpos <- preds$fpos*100
preds$fneg <- preds$fneg*100
...
joeNames <- data.frame(c("Mean BLL" ,
                         ...
                         "Percent latino",
                         "Mean age at first test (months)", 
                         "Median age at first test (months)"))
...
#################
# CONVERT AGE AT FIRST TEST TO MONTHS
#################
cen.df$meanAgeFirstTest <- cen.df$meanAgeFirstTest / 30.4375
cen.df$medianAgeFirstTest <- cen.df$medianAgeFirstTest / 30.4375
...
output$table1 = renderTable({
  rownum = which(preds$resp==input$resp & preds$model==input$model)
  outdf = matrix(as.integer(preds[rownum,3:6]), nrow=2, byrow=T)
  
  #rownames(outdf) = c("actual ABSENT", "actual PRESENT")
  rownames(outdf) <- c(paste0("Actual ", preds$response.name[which(preds$resp == input$resp)][1], " absent"), 
                       paste0("Actual ", preds$response.name[which(preds$resp == input$resp)][1], " present"))
  
  #colnames(outdf) = c("predicted ABSENT", "predicted PRESENT")
  colnames(outdf) <- c(paste0("Predicted ", preds$response.name[which(preds$resp == input$resp)][1], " absent"), 
                       paste0("Predicted ", preds$response.name[which(preds$resp == input$resp)][1], " present"))
...
output$splot2 = renderPlot(
  ...
  plot(preds$fpos[which(preds$resp == input$resp)], 
       1-preds$fneg[which(preds$resp == input$resp)],
       ...
       xlim=c(0,1),
       ylim=c(0,1),
       main=paste0("ROC curve for ", preds$response.name[which(preds$resp == input$resp)][1]))
  points(preds$fpos[which(preds$resp == input$resp &
