...
myLink <- "https://docs.google.com/spreadsheets/d/1sUYnuWQqtMq4w4o3PnxfRucgu-rITrsc2K---pD8WmE/export?&format=csv"

myCsv <- getURL(myLink)
preds <- read.csv(textConnection(myCsv))
rm(gpclibPermit, myCsv, myLink)
save.image("preds.RData") #Subho's model data
